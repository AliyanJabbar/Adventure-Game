...
console.log(`${chalk.bold.green(player.playerName)} VS ${chalk.bold.red(opponent.opponentName)}`);
let question = await inquirer.prompt({
    name: "select",
    type: "list",
    message: chalk.bold.italic.magenta("SELECT ONE OF THE FOLLOWING TO PROCEED..."),
    choices: [
        ...
        case (question.select == `WANT TO FIGHT WITH THE ${opponent.opponentName}`):
            {
                ...
                //  for opponent select
                let opponentSelect = Math.floor(Math.random() * 3) + 1;
                if (opponentSelect === 1) {
                ...
                if (playerSelect.select == opponentSelect) {
                    ...
                    console.log(chalk.italic.underline.red(`OPPONENT ALSO SELECTED ${opponentSelect}`));
                }
                if (playerSelect.select == "STONE" && opponentSelect == "PAPER") {
                    ...
                    console.log(chalk.bold.red(`${player.playerName}'s HEALTH IS: ${playerObj.health}`));
                    console.log(chalk.bold.green(`${opponent.opponentName}'s HEALTH IS: ${opponentObj.health}`));
                }
                if (playerSelect.select == "STONE" && opponentSelect == "SCISSOR") {
                    opponentObj.healthDecrease();
                    console.log(chalk.bold.underline(`OPPONENT SELECTED ${opponentSelect}`));
                    console.log(chalk.bold.green(`${player.playerName}'s HEALTH IS: ${playerObj.health}`));
                    console.log(chalk.bold.red(`${opponent.opponentName}'s HEALTH IS: ${opponentObj.health}`));
                }
                ...
                if (playerObj.health <= 0) {
                    console.log(`${chalk.magenta(opponent.opponentName)} KILLED THE ${chalk.magenta(player.playerName)}`);
                    console.log(chalk.bold.underline.bgRed(`${player.playerName} LOSE THE GAME!~!`));
                    condition = false;
                }
                if (opponentObj.health <= 0) {
                    console.log(`${chalk.magenta(player.playerName)} KILLED THE ${chalk.magenta(opponent.opponentName)}`);
                    console.log(chalk.bold.underline.bgGreen(`${player.playerName} WON THE GAME!~!`));
                    condition = false;
                }
            }
            break;
